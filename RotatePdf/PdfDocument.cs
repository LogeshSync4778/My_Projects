    ...
    public class PdfDocument
    {
        ...
        public void Save(string filename)
        {
            using (StreamWriter writer = new StreamWriter(filename))
            {
                ...
                {
                    writer.WriteLine($"{++objectCount} 0 obj");
                    writer.Write($"<< /Type /Page /Parent 2 0 R /MediaBox [0 0 612 792] /Resources 3 0 R /Rotate {pages[i].Rotation} /Contents {contentObjectStart + i} 0 R ");
                    writer.WriteLine(">>");
                    writer.WriteLine("endobj");
                }
                ...
                for (int i = 0; i < pages.Count; i++)
                {
                    string content = pages[i].GetContent();
                    writer.WriteLine($"{++objectCount} 0 obj");
                    writer.WriteLine("<< >>");
